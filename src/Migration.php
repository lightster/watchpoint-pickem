...
class Migration
{
    ...
    public static function createMigrationsTable(Db $db): bool
    {
        $sql = <<<SQL
...
TMPL;
    ...
    public static function runAll(App $app, Closure $run, Closure $done)
    {
    ...
    public static function rollback(App $app, Closure $run, Closure $done)
    {
        $db = $app->db();
        $db->query('BEGIN');
        $result = $db->query(
...
SQL;
    ...
    private function run(App $app, bool $rollback = false, int $batch = null)
    {
        ...
        if ($rollback === false) {
            ...
            $this->markAsUnrun($app->db());
        }
    }

    private function markAsRun(Db $db, int $batch)
    {
    ...
    private function markAsUnrun(Db $db)
    {
